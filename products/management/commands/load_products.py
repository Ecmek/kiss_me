    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for i in range(1, last_page + 1):
                ...
                soup = BeautifulSoup(response.text, 'lxml')
                products_card = soup.find_all('div', class_='product_data__gtm-js product_data__pageevents-js '
                                              'ProductCardHorizontal js--ProductCardInListing '
                                              'js--ProductCardInWishlist')
                for product in products_card:
                    product_price = product.find('span', class_='ProductCardHorizontal__price_current-price '
